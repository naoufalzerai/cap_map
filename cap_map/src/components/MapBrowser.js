...
export class MapBrowser extends Component {
    ...
    renderSeat = seat => {
        ...
        return  (
            ...
            <ListGroupItem  className="listGroup" hover uid ={seat.uid} name={seat.name} cords={seat.coords} type={seat.type} onClick={(e) => this.ClicSeats(seat,e)}>
    ...
    ClicSeats(seat,e){
       ...
       let type = seat.type ? seat.type : seat.target.type;
        switch(type){
            ...
            .attr("href", "#pointer")
            .attr("name", "pointer")
            .attr("transform", "translate(" +seatx + "," + seat.coords[1] + ")scale(0)")
            .attr("fill", "#039BE5")
            .attr("stroke", "#039BE5")
            .attr("stroke-width", "1px")
            .transition()
  ...
  ref={pan => (this.pan = pan)} >
    ...
    <div className="h-100 d-inline-block"></div>
        ...
        <Modal isOpen={this.state.modalAffect}>
          ...
          <ModalFooter>
            <Button color="secondary" onClick={this.close}>Close</Button>
            <Button color="primary" onClick={this.AffectValidation}>Affect to me</Button>
          </ModalFooter>
        </Modal>
